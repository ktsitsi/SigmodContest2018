...
std::string Joiner::join(QueryInfo&& qinfo){
	...
	root = new HashJoin(cleft,left_proj_colId,push_fil_l,cright,right_proj_colId,push_fil_r,1 << 10);
...
static void print_query(QueryInfo& qinfo){
	...
	for(auto& pred: qinfo.predicates){
		std::cerr<< pred.left.binding<<"."<<pred.left.colId<<"=" <<pred.right.binding<<"."<<pred.right.colId<<"&";
	}

	for(auto& fil: qinfo.filters){
		std::cerr<< fil.filterColumn.binding<<"."<<fil.filterColumn.colId<<'*'<<fil.constant<<" ";
	}
	std::cerr<<"|";

	for(auto& sel: qinfo.selections){
		std::cerr<< sel.binding<<"."<<sel.colId<<" ";
	}
	std::cerr<<"\n";
}
...
QueryInfo Joiner::inner_optimizer(std::set<unsigned>& S, QueryInfo& qinfo){
	...
	//print_query(qinfo);
	//std::cerr<<"Initialize Optimizer....."<<std::endl;
	...
	//std::cerr<<"Closing Optimizer....."<<std::endl;
	return reordered;
}
